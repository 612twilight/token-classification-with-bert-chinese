...
class NERTransformer(BaseTransformer):
    ...
    def __init__(self, hparams):
        ...
        try:
            token_classification_task_clazz = NER
        ...
        self.labels = self.token_classification_task.get_labels(os.path.join(hparams.data_dir, hparams.labels))
    ...
    def do_inference(self):
        self.eval()
        if not os.path.exists(os.path.join(args.data_dir, "inference.txt")):
            print("没有需要训练的数据")
        else:
            examples = self.token_classification_task.read_examples_from_file(args.data_dir, "inference")
            features = self.token_classification_task.convert_examples_to_features(
                examples,
                self.labels,
                args.max_seq_length,
            ...
            all_tokens = [example.words for example in examples]
            all_preds_list = []
            ...
            with open(os.path.join(args.data_dir, "out_inference.txt"), 'w', encoding='utf8') as writer:
                for sequence, pred_labels in zip(all_tokens, all_preds_list):
                    for token, label in zip(sequence, pred_labels):
...
if __name__ == "__main__":
    ...
    if args.do_inference:
        ...
        if not args.do_predict:
            checkpoints = list(
                sorted(glob.glob(os.path.join(args.output_dir, "checkpointepoch=*.ckpt"), recursive=True)))
            model = model.load_from_checkpoint(checkpoints[-1])
        model.do_inference()
